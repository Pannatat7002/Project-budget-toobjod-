...
class MakeKbankParser extends BankParserStrategy {
  @override
  String get bankId => 'make_kbank';

  @override
  String get bankName => 'MAKE by KBank';

  @override
  String get shortName => 'MAKE';

  @override
  int get brandColor => 0xFF2D8CFF; // Modern Blue

  @override
  List<String> get supportedPackages => const [
    'com.kasikornbank.makebykbank', // MAKE by KBank (Official)
    'com.kasikornbank.make',
    'com.kbank.make',
  ];
  ...
  static final RegExp _senderRegex = RegExp(
    r'(?:จาก|รับจาก|โอนจาก|Cloud Pocket จาก)\s*([A-Za-z0-9\u0E00-\u0E7F\s\.\-]+?)(?:\s+(?:จำนวน|ยอด|เข้า)|$|\s+[0-9])',
    caseSensitive: false,
  );
  ...
  DetectedTransaction? parse({
    ...
    } else if (lowerTitle.contains('โอนเงิน') || lowerTitle.contains('ชำระเงิน') || lowerTitle.contains('โอนออก')) {
      type = TransactionType.expense;
    } else {
      // ด่านที่ 2: ตรวจจับรูปแบบเฉพาะของ MAKE by KBank จาก Text
      final isMakeIncome = lowerText.contains('เงินเข้า') ||
          lowerText.contains('โอนเข้า') ||
          lowerText.contains('ได้รับเงิน');
      final isMakeExpense = lowerText.contains('โอนเงิน') ||
          lowerText.contains('ให้กับ') ||
          lowerText.contains('โอนออกจาก') ||
          lowerText.contains('ชำระให้');

      if (isMakeIncome && !isMakeExpense) {
        type = TransactionType.income;
      } else {
        type = TransactionType.expense;
      }
    ...
    final uniqueId = (id != null && id.isNotEmpty)
        ? id
        : 'tx_make_${timeBucket}_${amount.toStringAsFixed(2)}_${type.name}';
