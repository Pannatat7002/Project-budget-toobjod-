...
class AutoSyncCubit extends Cubit<AutoSyncState> {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      // Seed sample mock bank notifications if system is fresh and empty
      if (pending.isEmpty && transactionCubit.state.transactions.isEmpty) {
        debugPrint('[AutoSyncCubit] 🌱 Seeding initial sample bank notifications...');
        await mockSampleNotifications();
      }
  ...
  /// Mock / Simulate realistic incoming Thai Bank Notifications
  Future<int> mockSampleNotifications() async {
    final now = DateTime.now();

    final samples = [
      // 1. K PLUS (KBank) - Expense 7-Eleven
      {
      ...
      // 2. SCB EASY - Income เงินเดือน / รายได้
      {
      ...
      // 3. Krungthai NEXT - Expense ค่าเดินทาง BTS
      {
      ...
      // 4. ttb touch - Expense Cafe Amazon
      {
      ...
      // 5. K PLUS - Income โอนเงินรับ
      {
        'packageName': 'com.kasikorn.retail.mbanking.wap',
        'title': 'K PLUS',
        'text': 'เงินเข้า 1,200.00 บาท โอนจาก นายสมชาย ว. เข้า บช. x-4521',
    ...
    for (final sample in samples) {
      final detected = ThaiBankParser.parse(
        id: 'mock_${DateTime.now().microsecondsSinceEpoch}_${count + 1}',
        packageName: sample['packageName'] as String,
      ...
      if (detected != null) {
        await _handleIncomingDetectedTransaction(detected, bypassFilter: true);
        count++;
      }
    }
    return count;
  ...
  /// Simulate a single custom notification
  Future<bool> simulateNotification({
  ...
  /// Mock / Simulate realistic Bank SMS Notifications
  Future<int> mockSmsNotifications() async {
    final now = DateTime.now();
    ...
    int count = 0;
    for (final sample in smsSamples) {
      final detected = ThaiBankParser.parse(
        id: 'mock_sms_${DateTime.now().microsecondsSinceEpoch}_${count + 1}',
        packageName: sample['packageName'] as String,
        title: sample['title'] as String,
        text: sample['text'] as String,
        timestamp: sample['timestamp'] as DateTime,
